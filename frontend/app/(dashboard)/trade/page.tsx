...
export default function TradePage() {
    ...
    // Data State
    const [livePrice, setLivePrice] = useState(150.25)
    ...
    useEffect(() => {
        ...
        const fetchPrice = async () => {
            try {
                ...
                if (data && isMounted) {
                    setLivePrice(data.last_price || data.price || data.currentPrice || 150.25);
                }
            } catch (err) {
    ...
    const unrealizedPnl = portfolio?.positions ? 
        portfolio.positions.reduce((acc: number, p: any) => acc + p.unrealized_pnl, 0) : 0;
        ...
        <div className="flex flex-col h-screen bg-[#121212] text-slate-300 font-sans overflow-hidden">
            ...
            <div className="flex items-center justify-between p-4 border-b border-slate-800 bg-[#1a1a1a]/50 backdrop-blur-md z-10">
                ...
                <div className="flex items-center gap-4 text-xs font-mono">
                    ...
                    <span className="text-slate-500">MS: 12ms</span>
                </div>
            </div>
